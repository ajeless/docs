...
**Status:** decided (structure), deferred (visual design)
**Scope:** v0.1 vertical slice
**Last updated:** 2026-04-20

## Summary

The v0.1 interface is a two-panel layout at golden ratio (~38:62) split: ship schematic on the left, tactical viewport on the right. A thin top bar shows the current phase and timer; a bottom strip combines movement readouts with action buttons. The ship schematic is an interactive control surface where systems are placed on the hull at their physical locations. The tactical viewport is a minimalist sensor-style display. This document captures the *structural* decisions; visual design (colors, typography, exact dimensions) is deferred.
...
- **Readouts** (information-only displays) — flat, no borders suggesting buttonness, typography leaning numeric/monospace where appropriate. Hull/reactor/heat tiles at the top of the schematic pane are readouts. The log strip is a readout.
...
A horizontal strip below the two main panels combines numeric readouts on the left (thrust budget, velocity, heading) with action buttons on the right (Clear, Pass, Submit plot). Readouts and buttons are visually distinguished within the strip — readouts flat, buttons bordered.
...
Clicking a system selects it, entering a context-sensitive mode. For weapon mounts, this is **aim mode** — the tactical viewport shows a firing arc, and clicking a target fires the weapon. For other systems, the click shows the system's current status and any actionable options (power allocation, damage control assignment, etc. — v0.1 doesn't exercise this for non-mount systems but the interaction pattern is established).

### Status tiles inside the schematic viewport

The schematic viewport's top-left corner contains a small readout panel showing the ship's overall status — hull %, reactor output, heat. This is redundant with visual information on the schematic itself (a damaged reactor shows color changes on the reactor button), but the numeric form is scannable faster.
...
- A dashed elliptical reachable region showing where the ship can be at the end of the next turn given its current velocity and thrust budget.
...
The thrust plot lives on the tactical viewport. Dragging handles updates the ghost position in real-time; the reachable region stays constant (it's the envelope of what's possible).
...
- **Plot phase (default).** The state shown in the reference wireframe. Player plots thrust and weapon intent. Top bar is neutral dark.
- **Aim mode.** Entered by clicking a weapon mount on the schematic. Mount is highlighted and visually "selected." Tactical viewport shows firing arc overlay from the mount's position. Clicking an enemy ship in arc fires the weapon. Top bar is amber. An Esc hotkey cancels aim mode and returns to plot phase.
...
- `idea_capture.md` (in sibling folder) — preserved design context for the SSD and UI philosophy.
