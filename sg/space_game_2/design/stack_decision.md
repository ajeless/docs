...
The host runs the server locally and exposes it via a tunnel. Free tier is fine for friends-only play. No dedicated hosting, no DNS, no TLS management.
...
- `idea_capture.md` in the sibling `space_game_1/` folder — the preserved ideation that produced these decisions (not binding).
- `ship_definition_format.md` *(next)* — the data format for ships, which the resolver consumes and the UI renders.
- `resolver_design.md` *(next)* — the engine that these stack choices exist to support.
- `ssd_layout.md` *(next)* — the user-facing interface structure.